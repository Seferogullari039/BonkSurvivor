...
[InitializeOnLoad]
public static class TankEnemyViewPrefabBuilder
{
    private const string SkeletonModelPath =
        "Assets/Art/Characters/Enemies/Quaternius_LowPolyAnimatedMonsters/FBX/Skeleton.fbx";
    private const string PrefabsFolder = "Assets/Prefabs/Enemies";
    private const string MaterialsFolder = PrefabsFolder + "/Materials";
    private const string PrefabPath = PrefabsFolder + "/TankEnemy_View.prefab";
    private const string BodyMaterialPath = MaterialsFolder + "/TankEnemy_Body_Mat.mat";
    private const float TargetHeight = 1.22f;
    private const float VisualGroundLocalY = -0.456f;

    private static bool autoBuildAttempted;

    static TankEnemyViewPrefabBuilder()
    {
        EditorApplication.delayCall += TryBuildMissingPrefab;
    }

    [MenuItem("Tools/BonkSurvivor/Build Tank Enemy View Prefab", false, 24)]
    public static void BuildTankEnemyViewPrefab()
    {
        if (!AssetPathExists(SkeletonModelPath))
        {
            Debug.LogWarning("[TankEnemyViewPrefabBuilder] Skeleton model missing at " + SkeletonModelPath);
            return;
        }

        if (!EnsureSkeletonImported(out string importIssue))
        {
            Debug.LogWarning("[TankEnemyViewPrefabBuilder] Skeleton import not ready: " + importIssue);
            return;
        }
        ...
        {
            Debug.LogWarning("[TankEnemyViewPrefabBuilder] Prefab build failed: " + buildIssue);
            return;
        }
        ...
        if (prefab != null)
        {
            Debug.Log("[TankEnemyViewPrefabBuilder] TankEnemy_View prefab ready at " + PrefabPath);
            EditorGUIUtility.PingObject(prefab);
        }
    }

    [MenuItem("Tools/BonkSurvivor/Build Tank Enemy View Prefab", true)]
    private static bool ValidateBuildTankEnemyViewPrefab()
    {
    ...
    {
        issue = string.Empty;
        GameObject source = ResolveModelSource(SkeletonModelPath);

        if (source == null)
        {
            issue = "Could not load Skeleton FBX source.";
            return false;
        }
        ...
        GameObject prefabRoot = new GameObject("TankEnemy_View");
        ...
        if (modelObject == null)
        {
            ...
            issue = "Could not instantiate Skeleton model.";
            return false;
        }
    ...
    private static bool EnsureSkeletonImported(out string issue)
    {
        issue = string.Empty;

        AssetDatabase.ImportAsset(SkeletonModelPath, ImportAssetOptions.ForceUpdate);
        AssetDatabase.Refresh(ImportAssetOptions.ForceUpdate);

        AssetImporter importer = AssetImporter.GetAtPath(SkeletonModelPath);
        ...
        if (source == null)
        {
            issue = "Skeleton FBX did not import as GameObject.";
            return false;
        }
        ...
        {
            issue = "Skeleton FBX imported without renderers.";
            return false;
        }
    ...
    {
        CreateOrUpdateMaterial(BodyMaterialPath, new Color(0.46f, 0.12f, 0.72f), 0.44f, false);
    }
    ...
    private static void CreateOrUpdateMaterial(string assetPath, Color baseColor, float smoothness, bool emission)
    {
        ...
        if (shader == null)
        {
            Debug.LogError("[TankEnemyViewPrefabBuilder] Lit shader not found for " + assetPath);
            return;
        }
