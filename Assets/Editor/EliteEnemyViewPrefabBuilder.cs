...
[InitializeOnLoad]
public static class EliteEnemyViewPrefabBuilder
{
    ...
    private const string PrefabPath = PrefabsFolder + "/EliteEnemy_View.prefab";
    private const string BodyMaterialPath = MaterialsFolder + "/EliteEnemy_Body_Mat.mat";
    private const float TargetHeight = 1.16f;
    private const float VisualGroundLocalY = -0.456f;

    private static bool autoBuildAttempted;

    static EliteEnemyViewPrefabBuilder()
    {
        EditorApplication.delayCall += TryBuildMissingPrefab;
    }

    [MenuItem("Tools/BonkSurvivor/Build Elite Enemy View Prefab", false, 25)]
    public static void BuildEliteEnemyViewPrefab()
    {
        if (!AssetPathExists(DragonModelPath))
        {
            Debug.LogWarning("[EliteEnemyViewPrefabBuilder] Dragon model missing at " + DragonModelPath);
            return;
        }

        if (!EnsureDragonImported(out string importIssue))
        {
            Debug.LogWarning("[EliteEnemyViewPrefabBuilder] Dragon import not ready: " + importIssue);
            return;
        }
        ...
        {
            Debug.LogWarning("[EliteEnemyViewPrefabBuilder] Prefab build failed: " + buildIssue);
            return;
        }
        ...
        if (prefab != null)
        {
            Debug.Log("[EliteEnemyViewPrefabBuilder] EliteEnemy_View prefab ready at " + PrefabPath);
            EditorGUIUtility.PingObject(prefab);
        }
    }

    [MenuItem("Tools/BonkSurvivor/Build Elite Enemy View Prefab", true)]
    private static bool ValidateBuildEliteEnemyViewPrefab()
    {
    ...
    private static bool BuildPrefab(out string issue)
    {
        ...
        GameObject prefabRoot = new GameObject("EliteEnemy_View");
    ...
    {
        CreateOrUpdateMaterial(BodyMaterialPath, new Color(1f, 0.82f, 0.1f), 0.46f, false);
    }
    ...
    private static void CreateOrUpdateMaterial(string assetPath, Color baseColor, float smoothness, bool emission)
    {
        ...
        if (shader == null)
        {
            Debug.LogError("[EliteEnemyViewPrefabBuilder] Lit shader not found for " + assetPath);
            return;
        }
